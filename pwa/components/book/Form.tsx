...
export const Form: FunctionComponent<Props> = ({book}) => {
  ...
  const saveMutation = useMutation<FetchResponse<Book> | undefined,
    Error | FetchError,
    SaveParams>((saveParams) => saveBook(saveParams));

  const deleteMutation = useMutation<FetchResponse<Book> | undefined,
    Error | FetchError,
    DeleteParams>(({id}) => deleteBook(id), {
    onSuccess: () => {
      router.push("/books");
    },
    onError: (error) => {
